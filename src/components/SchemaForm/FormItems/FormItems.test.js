...
const getByRole = (name) => screen.getByRole('gridcell', { name });

const renderFormItems = ({ isStateInitial = true, renderProps = {} } = {}) => {
    const handleRemoveOnItemsRowClick = jest.fn();
    const onValidateHandler = jest.fn();
    const schema = isStateInitial ?
        { items: [] }
    : { items: [{ id: 1, name: 'Name 1', quantity: 'Quantity 1' }] };
    ...
    render(
        <Form
            ...
            render={({ initialValues }) => (
                <FormItems
                    handleRemoveOnItemsRowClick={handleRemoveOnItemsRowClick}
...
test('FormItems is not displayed without items', () => {
    renderFormItems();

    expect(screen.queryByRole('gridcell')).not.toBeInTheDocument();
});

test('FormItems is displayed with items', () => {
    renderFormItems({ isStateInitial: false });
screen.debug()
...
//     const { addPlate, removePlate, submitPlate, resetPlate, cancelPlate } = renderFormItems();
...
//     const { addPlate, removePlate } = renderFormItems();
